...
<?php
    ...
    try {
        $stmt = $connect->prepare("SELECT  ender.id, ender.dia, ender.horario, ender.comquem, ender.assunto, ender.topico FROM cdsagenda as ender;");
        $stmt->execute();

        $cdsagenda = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        $result["data"] = $cdsagenda; //criamos o array para devolver o resultado do insert com os dados inseridos.
